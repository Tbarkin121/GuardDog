/**
  ...
  *          topology is used. Differently from standard implementation, 
  *          this one performs multiple acquisition during a PWM cycle.
  *
...
__weak void BADC_Init( Bemf_ADC_Handle_t *pHandle)
{
  ...
  if ( ( uint32_t )pHandle == ( uint32_t )&pHandle->_Super )
  {
    ...
    LL_DMA_SetDataLength(DMA1, LL_DMA_CHANNEL_1, pHandle->OverSamplingRate );
    ...
    LL_TIM_CC_EnableChannel( pHandle->pParams_str->TIM_Trigger, pHandle->pParams_str->TIM_ADC_Trigger_Channel );
    LL_TIM_EnableCounter( pHandle->pParams_str->TIM_Trigger );
    tArray = LL_TIM_GetAutoReload(pHandle->pParams_str->TIM_Trigger);
    LL_TIM_OC_SetCompareCH1( pHandle->pParams_str->TIM_Trigger, pHandle->pSensing_Params->SamplingPoint);

    for (i=0; i<pHandle->OverSamplingRate; i++)
    {
      pHandle->TIM1_pulses[i]= tArray * i + pHandle->Pwm_OFF.SamplingPoint;
    }
...
{
  uint8_t i;
  uint32_t DeltaCounter, tZC_Counter = pHandle->ZC_Counter_On_Last;
  ...
  if ( pHandle->DemagCounter >= pHandle->DemagCounterThreshold)
  {
    if (pHandle->ZcDetected == false)
    {
      for (i=0; i<pHandle->OverSamplingRate; i++)
      {
        if (((pHandle->TIM1_pulses[i] > (pHandlePWMC->CntPh + pHandle->SamplingGuard)) && (false == pHandle->IsOnSensingEnabled))
            || ((pHandle->TIM1_pulses[i] < (pHandlePWMC->CntPh - pHandle->SamplingGuard)) && (true == pHandle->IsOnSensingEnabled)))
        {
          switch(pHandlePWMC->Step)
          {
          case STEP_1:
            pHandle->BemfLastValues[2] = pHandle->ADC1_DMA_converted[i];
            if(pHandle->Direction == 1)
            {
              if (pHandle->ADC1_DMA_converted[i] < pHandle->pSensing_Params->AdcThresholdDown)
              {
                ZcDetection = true;
            ...
            {
              if (pHandle->ADC1_DMA_converted[i] > pHandle->pSensing_Params->AdcThresholdUp)
              {
                ZcDetection = true;
            ...
            break;
          case STEP_2:
            pHandle->BemfLastValues[1] = pHandle->ADC1_DMA_converted[i];
            if(pHandle->Direction == 1)
            {     
              if (pHandle->ADC1_DMA_converted[i] > pHandle->pSensing_Params->AdcThresholdUp)
              {
            ...
            break;      
          case STEP_3:
            pHandle->BemfLastValues[0] = pHandle->ADC1_DMA_converted[i];
            if(pHandle->Direction == 1)
            {
              if (pHandle->ADC1_DMA_converted[i] < pHandle->pSensing_Params->AdcThresholdDown)
              {
          ...
          if (ZcDetection == true)
          {
            DeltaCounter = pHandle->TIM1_pulses[pHandle->OverSamplingRate - 1] - pHandle->TIM1_pulses[i];
            break;
          }
        }
        else if ((pHandle->TIM1_pulses[i] < (pHandlePWMC->CntPh - pHandle->SamplingGuard)) && (false == pHandle->IsOnSensingEnabled) && (pHandle->ZcDetected == false))
        {
          tZC_Counter = pHandle->ADC1_DMA_converted[i];
        }
      }
    }
    if (true == ZcDetection)
    {
      pHandle->ZC_Counter_On_Last = tZC_Counter;
      ...
      case STEP_5:
        if(pHandle->Direction == 1)
        {     
          pHandle->ZC_Counter_Down = LL_TIM_GetCounter(pHandle->pParams_str->LfTim) - DeltaCounter/(pHandle->LowFreqTimerPsc + 1);
          pHandle->ZC_Counter_Last = pHandle->ZC_Counter_Down;
        }
        else
        {
          pHandle->ZC_Counter_Up = LL_TIM_GetCounter(pHandle->pParams_str->LfTim) - DeltaCounter/(pHandle->LowFreqTimerPsc + 1);
