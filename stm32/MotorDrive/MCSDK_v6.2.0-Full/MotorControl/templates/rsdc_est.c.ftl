...
<#if MC.M1_SPEED_SENSOR == "ZEST">          
      ...
      if ((MCM_SPEED_MODE == pFOCVars->controlMode) ||
          (MCM_TORQUE_MODE == pFOCVars->controlMode))
      {
        pRsDCEst->pSPD->closedLoopAngleEnabled = true;
      }
      else
      {
        /* nothing to do */
...
void RsDC_estimationFiltering(RsDCEstimation_Handle_t *pRsDCEst,   Currents_Iab_t   *Iab_in_pu,  Voltages_Uab_t   *Uab_in_pu)
{
  ...
  pRsDCEst->Iab_inLP_pu.A += ((Iab_in_pu->A - pRsDCEst->Iab_inLP_pu.A) >> 10);
  pRsDCEst->Iab_inLP_pu.B += ((Iab_in_pu->B - pRsDCEst->Iab_inLP_pu.B) >> 10);
  pRsDCEst->Uab_inLP_pu.A += ((Uab_in_pu->A - pRsDCEst->Uab_inLP_pu.A) >> 10);
  pRsDCEst->Uab_inLP_pu.B += ((Uab_in_pu->B - pRsDCEst->Uab_inLP_pu.B) >> 10);
}
