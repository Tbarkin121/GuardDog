...
#define DYNAMIC_Q_GAIN_FLAG      (1 << DYNAMIC_Q_GAIN_SHIFT)
    ...
    case MC_REG_START_CONFIGURATION:
      {
        ...
        pMCI->pSPD->flagDynamicQgain = regdata16 & DYNAMIC_Q_GAIN_FLAG;
        ...
        case MC_REG_START_CONFIGURATION:
          {
            ...
            startFlags |= pMCI->pSPD->flagDynamicQgain << DYNAMIC_Q_GAIN_SHIFT;
...
<#if MC.M1_SPEED_SENSOR == "ZEST">           
        case MC_REG_FOC_ID_INJECT_A:
          *regdata32 = FIXP30_mpy(pMCI->pSPD->zestControl.injectId_A_pu, FIXP16(scaleParams->current));
          break;
        case MC_REG_FOC_INJECTFREQ_HZ:
          *regdata32 = FIXP30_mpy(pMCI->pSPD->zestControl.injectFreq_kHz, FIXP16(1000.0f)); /* bRetVal = FIXP16(Hz) */
          break;  
        case MC_REG_FOC_GAIN_D:
...
uint8_t flashRead_cb (uint16_t rxLength, uint8_t *rxBuffer, int16_t txFreeSpace, uint16_t *txLength, uint8_t *txBuffer)
{
  ...
  if (sizeof(FLASH_Params_t) <= txFreeSpace)
  {
    memcpy (txBuffer, &flashParams, sizeof(FLASH_Params_t));
    *txLength = sizeof(FLASH_Params_t);
  }
  else
  {
