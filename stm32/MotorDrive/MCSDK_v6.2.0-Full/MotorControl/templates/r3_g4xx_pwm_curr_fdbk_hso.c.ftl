...
TIM_TypeDef * TIMx_Oversample = pHandle->pParams_str->TIMx_Oversample;
  ...
  if (pHandle->isShorting)
  {
    ...
    /* Put 50% duty on the phases. */
    // Phase voltage measurements are improved
    duty = pHandle->Half_PWMPeriod >> 1;
...
<#if MC.M1_POTENTIOMETER_ENABLE == true >
	...
	*pAuxMeasurement = FIXP_MPY(sample_Aux, FIXP30(1.0/(OVS_COUNT * REGULATION_EXECUTION_RATE)), ADC_FMT);
<#else>
	/* No Aux */
	*pAuxMeasurement = 0;
</#if>
