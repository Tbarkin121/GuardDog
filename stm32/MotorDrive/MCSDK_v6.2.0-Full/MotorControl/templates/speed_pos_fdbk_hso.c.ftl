...
<#else>
  ...
  pHandle->DirCheck_threshold_pu = FIXP30(2.0f / Hso_params_M1.FullScaleFreq_Hz);
  
<#if MC.M1_SPEED_SENSOR == "ZEST">  

  /* Initialize the structures used to control ZEST */
  FOC_ZESTControl_s* pControl = &pHandle->zestControl;
  pControl->injectId_A_pu = FIXP30(0.0f / flashParams->scale.current);
  pControl->injectIq_A_pu = FIXP30(0.0f / flashParams->scale.current);
  ...
  pControl->feedbackGainD = FIXP24(flashParams->zest.zestGainD);
  pControl->feedbackGainQ = FIXP24(flashParams->zest.zestGainQ);
...
void MC_ZEST_Control_Update(SPD_Handle_t *pHandle)
{
  ...
  if (pHandle->flagDynamicQgain)
  {
    if(RSEST_getCurrentIsLow(pHandle->pRsEst) == false)
    {
      gainQ = FIXP(0.0f);
    }
    else
