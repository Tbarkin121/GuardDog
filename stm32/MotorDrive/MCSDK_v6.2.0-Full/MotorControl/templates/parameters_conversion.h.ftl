...
<#assign ADC_tab = [ MC.M1_CS_ADC_U, MC.M1_CS_ADC_V, MC.M1_CS_ADC_W, MC.M1_VS_ADC_U, MC.M1_VS_ADC_V, MC.M1_VS_ADC_W, MC.M1_VBUS_ADC, MC.M1_TEMP_FDBK_ADC, MC.M1_POTENTIOMETER_ADC] >
<#assign ADC1Length = 0 >
<#assign ADC2Length = 0 >
<#assign ADC3Length = 0 >
<#assign ADC4Length = 0 >
...
/* Current conversion from Ampere unit to 16Bit Digit */
<#if MC.M2_DRIVE_TYPE == "FOC" && MC.DRIVE_NUMBER != "1">
...
#define M2_MAX_READABLE_CURRENT2 (ADC_REFERENCE_VOLTAGE / (2 * RSHUNT2 * AMPLIFICATION_GAIN2))
</#if>
...
#define M1_ICL_CAPS_CHARGING_DELAY_TICKS       (uint16_t)(((M1_ICL_CAPS_CHARGING_DELAY_MS *\
                                               MEDIUM_FREQUENCY_TASK_RATE) / 1000) - 1u)
#define M1_ICL_VOLTAGE_THRESHOLD_U16VOLT       (uint16_t)((M1_ICL_VOLTAGE_THRESHOLD * 65536U)\
...
<#if MC.M1_SPEED_SENSOR == "HSO" || MC.M1_SPEED_SENSOR == "ZEST">
<#if MC.M1_CS_ADC_NUM == "2" >
#define TW_BEFORE ((uint16_t)((ADC_TRIG_CONV_LATENCY_CYCLES + ADC_SAMPLING_CYCLES*2 + ADC_SAR_CYCLES) * ADV_TIM_CLK_MHz) / ADC_CLK_MHz  + 1u)
<#else>
...
#define MAX_TICKS_CURRENT_RECONSTRUCTION_THRESHOLD ((((CURRENT_RECONSTRUCTION_THRESHOLD + 1.0)/2.0)) * PWM_PERIOD_CYCLES / 2)

  <#if (MC.M1_AMPLIFICATION_GAIN?number >0)> <#assign ampSign = "-"> <#else> <#assign ampSign = ""> </#if>
#define ADC_CURRENT_SCALE (${ampSign}(float_t)ADC_REFERENCE_VOLTAGE/(float_t)AMPLIFICATION_GAIN/(float_t)RSHUNT)
#define ADC_VOLTAGE_SCALE ((float_t)ADC_REFERENCE_VOLTAGE/(float_t)M1_PHASE_VOLTAGE_PARTITIONING_FACTOR)
...
#define OVS_RANK_U_DC     OVS_RANK${MC.M1_VBUS_ADC_RANK} /*!< @brief rank of the ADC channel used to sample DC Bus */
  ...
  <#if MC.M1_POTENTIOMETER_ENABLE>
    <#if (MC.M1_POTENTIOMETER_ADC ==  MC.M1_CS_ADC_U) || (MC.M1_POTENTIOMETER_ADC ==  MC.M1_CS_ADC_V) || (MC.M1_POTENTIOMETER_ADC ==  MC.M1_CS_ADC_W)>
#define OVS_ADC_THROTTLE  OVS_${MC.M1_POTENTIOMETER_ADC} /*!< @brief ADC used to sample throttle/potentiometer */
...
#define OVERSAMPLING_FIXP_CURRENT_FACTOR(Currentscale)    (FIXP30(CURRENT_FACTOR(Currentscale) / (OVS_COUNT * REGULATION_EXECUTION_RATE)))
  <#else>
/* Factor for one sample per period for shunt */
#define OVERSAMPLING_FIXP_CURRENT_FACTOR(Currentscale)    (FIXP30(CURRENT_FACTOR(Currentscale) / REGULATION_EXECUTION_RATE))
  </#if>
#define OVERSAMPLING_FIXP_VOLTAGE_FACTOR(VoltageScale)  (FIXP30(VOLTAGE_FACTOR(VoltageScale) / (OVS_COUNT * REGULATION_EXECUTION_RATE)))
#define OVERSAMPLING_FIXP_BUSVOLTAGE_FACTOR(VoltageScale)  (FIXP30(BUSVOLTAGE_FACTOR(VoltageScale) / (OVS_COUNT * REGULATION_EXECUTION_RATE)))

/* Index of the sample used for single-sampled channels, the low side shunt measurements  */
#define BOARD_SHUNT_SAMPLE_SELECT ((OVS_COUNT/2) - 1)  /* use last sample */
</#if>
