...
/*! @brief  delay to wait state machine is in stop state (expressed in ms)*/ 
#define STOPPERMANENCY_MS  400

/*! @brief  delay to wait state machine is in stop state (expressed in number of Systick interrupt) */
...
<#if (MC.M1_DBG_OPEN_LOOP_ENABLE == false) && (MC.MOTOR_PROFILER == false)> 
  ...
  FOCVars[M1].controlModeDefault = DEFAULT_CONTROL_MODE;
...
<#if  MC.M1_ICL_ENABLED == true>
  ICL_Init(&ICL_M1, &VBus_M1, &ICLDOUTParamsM1);
...
void TSK_MF_StopProcessing(  MCI_Handle_t * pHandle, uint8_t motor)
{
  ...
  FOCVars[motor].controlModeDefault = FOCVars[motor].controlMode;
  FOC_Clear(motor);
...
<#if MC.M1_SPEED_SENSOR == "ZEST">
  ...
  {
    ZEST_runBackground(&ZeST_M1, HSO_getEmfSpeed_pu(&HSO_M1));
  ...
  /* Resistance update during RUN state*/
  {
    ...
    {    
      RSEST_runBackSlowed(&RsEst_M1, ZEST_getFlagInjectActive(&ZeST_M1),  ZEST_getCheckRs(&ZeST_M1), ZEST_getR(&ZeST_M1));
    }
...
<#if (MC.M1_ICL_ENABLED == true)>
  ...
  <#if (MC.M1_PWM_DRIVER_PN == "STDRIVE101") && (MC.M1_DP_TOPOLOGY != "NONE") >
            ...
            if (pFOCVars->flagStartWithCalibration == true)
            {
...
<#if MC.M1_OTF_STARTUP == false >
              ...
              R3_TurnOnLowSides(pwmcHandle[M1], M1_CHARGE_BOOT_CAP_DUTY_CYCLES);  
              Mci[M1].State = CHARGE_BOOT_CAP;    
<#else> <#-- MC.M1_OTF_STARTUP = true -->
              FOC_Clear(M1);
  <#if (MC.M1_DBG_OPEN_LOOP_ENABLE == false) && (MC.MOTOR_PROFILER == false)> 
              MCI_SetControlMode(&Mci[M1],FOCVars[M1].controlModeDefault);
  </#if>                                  
...
<#if MC.M1_OTF_STARTUP == false >
...
</#if>
            {
                ...
                if(true == RsDCEst_M1.flag_enableRSDCestimate)
                {
<#if (MC.M1_PWM_DRIVER_PN == "STDRIVE101") && (MC.M1_DP_TOPOLOGY != "NONE") >
                  /* deactivate driver protection input while STDRIVE101 exiting standby mode */
                  LL_TIM_Disable${brkNb}(${_last_word(MC.M1_PWM_TIMER_SELECTION)});
                  ...
                  Mci[M1].State = STATE_RSDCESTIMATE; /* Proceed on Rs estimation  */
                }
                else
                {
<#if (MC.M1_DBG_OPEN_LOOP_ENABLE == false) && (MC.MOTOR_PROFILER == false)> 
      ...
      case RUN:
      {
        ...
        else
        {
          if (RsDCEst_M1.flag_enableRSDCestimate && RsDCEst_M1.RSDCestimate_state != RSDC_EST_COMPLETED &&
              ...
              ((pFOCVars->controlMode_prev == MCM_OBSERVING_MODE) || (pFOCVars->controlMode_prev == MCM_SHORTED_MODE) ||
               (pFOCVars->controlMode_prev == MCM_PROFILING_MODE)) &&
...
<#if (MC.M1_PWM_DRIVER_PN == "STDRIVE101") && (MC.M1_DP_TOPOLOGY != "NONE") >
          ...
          else if ( (MC_PolPulse_M1.flagPolePulseActivation) &&
                    (POLPULSE_getState(&MC_PolPulse_M1.PolpulseObj) != POLPULSE_STATE_PostcalcsComplete) &&
                    (pFOCVars->controlMode == MCM_SPEED_MODE || pFOCVars->controlMode == MCM_TORQUE_MODE) &&
                    (pFOCVars->controlMode_prev == MCM_OBSERVING_MODE || pFOCVars->controlMode_prev == MCM_SHORTED_MODE ||
                     pFOCVars->controlMode_prev == MCM_PROFILING_MODE ) &&
                    (pFOCVars->controlMode_prev != pFOCVars->controlMode) )
...
<#if (MC.MOTOR_PROFILER == true)>   
                case MCM_PROFILING_MODE:
                  ...
                  MCI_SetMaxCurrent(&Mci[M1],FIXP16(0.8f*BOARD_MAX_CURRENT));
                  STC_M1.SpeedControlEnabled = false;
                  CurrCtrl_M1.currentControlEnabled = false;
                  SPD_M1.closedLoopAngleEnabled = false;
...
<#if MC.M1_SPEED_SENSOR == "ZEST">
      ...
      case STATE_RSDCESTIMATE:
      {
        ...
        {
          RsDC_estimationRun(pFOCVars, &RsDCEst_M1);
          if (RSDC_EST_COMPLETED == RsDCEst_M1.RSDCestimate_state)
          {
...
<#else>
          ...
          {
            /* wait for Rs DC estimation completion */
          }
        }
        break; 
      }
...
__weak void FOC_Clear(uint8_t bMotor)
{
  ...
  pFOCVars->controlMode = MCM_OBSERVING_MODE;
  ...
  pCurrCtrlhandle[bMotor]->Ddq_ref_pu.D = FIXP30(0.0f);
  pCurrCtrlhandle[bMotor]->Ddq_ref_pu.Q = FIXP30(0.0f);

<#if (MC.M1_DBG_OPEN_LOOP_ENABLE == true) && (MC.MOTOR_PROFILER == true)> 
  pSTC[bMotor]->Idq_ref_pu.D = FIXP30(0.0f);
  pSTC[bMotor]->Idq_ref_pu.Q = FIXP30(0.0f);
  pSTC[bMotor]->speed_ref_pu = FIXP30(0.0f);
  pSTC[bMotor]->speed_ref_ramped_pu = FIXP30(0.0f);
</#if>
...
<#if MC.M1_SPEED_SENSOR == "ZEST">
  ...
  {
    CurrCtrlInp.Idq_ref_currentcontrol_pu.D += SensorlessOut.Idq_inject_pu.D;
    CurrCtrlInp.Idq_ref_currentcontrol_pu.Q += SensorlessOut.Idq_inject_pu.Q;
  }
</#if>  
