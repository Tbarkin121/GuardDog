...
<#if MC.M1_POTENTIOMETER_ENABLE> 
  <#if (MC.M1_POTENTIOMETER_ADC ==  MC.M1_CS_ADC_U) || (MC.M1_POTENTIOMETER_ADC ==  MC.M1_CS_ADC_V) || (MC.M1_POTENTIOMETER_ADC ==  MC.M1_CS_ADC_W)>
    /* Throttle */
    {
      ...
      .tasklen = OVS_${MC.M1_POTENTIOMETER_ADC}_TASKLENGTH,       
    },
</#if>   
</#if>  
...
RsDCEstimation_Handle_t RsDCEst_M1 =
{
  ...
  .RSDCestimate_AlignOnly = true,
  .RSDCestimate_counterInitialValue = (uint32_t) (1.0f * SPEED_LOOP_FREQUENCY_HZ), /* seconds */
  .RSDCestimate_counterMeasureValue = (uint32_t) ((1.0f - 0.75f) * (uint32_t) (1.0f * SPEED_LOOP_FREQUENCY_HZ)),
  .RSDCestimate_Id_ref_pu = FIXP30(NOMINAL_CURRENT_A * 0.5f / (float_t)CURRENT_SCALE),
  .RSDCestimate_rampRate = FIXP30_mpy((FIXP30(NOMINAL_CURRENT_A * 0.5f / (float_t)CURRENT_SCALE)), FIXP30(1.0f / ((uint32_t) ((1.0f - 0.75f) * (uint32_t) (1.0f * SPEED_LOOP_FREQUENCY_HZ))))),
...
<#if MC.M1_SPEED_SENSOR == "ZEST">
  ...
  .flagDynamicQgain = ${MC.MOTOR_DYNAMICQGAIN?c}, 
...
PROFILER_Params profilerParams_M1 =
{
  ...
  .dcac_dc_measurement_time_s = 0.7f,
<#if MC.M1_SPEED_SENSOR == "HSO">
  .dcac_ac_measurement_time_s = 5.0f,
<#else>
  .dcac_ac_measurement_time_s = 0.7f,
</#if>  
  .dcac_duty_maxgoal = 0.2f,
  .dcac_ac_freq_Hz = 80.0f,
  .dcac_DCmax_current_A = NOMINAL_CURRENT_A*0.8f,	/* max current, power_goal should come first */
