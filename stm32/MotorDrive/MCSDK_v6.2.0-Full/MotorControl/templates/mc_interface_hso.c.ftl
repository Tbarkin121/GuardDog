...
/**
  ...
  * 
  */
__weak MC_RetStatus_t MCI_StartMotor( MCI_Handle_t * pHandle )
{
  uint32_t RetVal = MC_WRONG_STATE_ERROR;

  if (MCI_GetOffsetsCalibrationState(pHandle) != COMPLETED)
  {
    RetVal = MC_NO_POLARIZATION_OFFSETS_ERROR;
  }
  else if ((IDLE == MCI_GetSTMState(pHandle)) &&
...
/**
  ...
  * 
  */
__weak MC_RetStatus_t MCI_StartOffsetMeasurments(MCI_Handle_t *pHandle)
{
  bool RetVal;
...
/**
  ...
  * the target motor and returns immediately. It returns #MC_SUCCESS if successful, or 
  ...
  * 
  * @param  pHandle Pointer on the component instance to work on.
  * @retval MC_RetStatus_t returns #MC_SUCCESS if successfully executed, or an
  *         error code otherwise.
  */
__weak MC_RetStatus_t MCI_StartWithPolarizationMotor( MCI_Handle_t * pHandle )
{
  uint32_t RetVal = MC_WRONG_STATE_ERROR;
...
/**
  ...
  * @retval returns #MC_SUCCESS if successfully executed, or an error code otherwise.
  */
__weak MC_RetStatus_t MCI_StopMotor( MCI_Handle_t * pHandle )
{
  MC_RetStatus_t RetVal;
  bool status;
  MCI_State_t State;
...
MC_RetStatus_t MCI_SetCalibratedOffsetsMotor( MCI_Handle_t* pHandle, PolarizationOffsets_t * PolarizationOffsets)
{
  uint32_t RetVal = MC_WRONG_STATE_ERROR;
...
MC_RetStatus_t MCI_StartOffsetsCalibrationMotor(MCI_Handle_t* pHandle)
{
   uint32_t RetVal = MC_WRONG_STATE_ERROR;
...
MC_RetStatus_t MCI_SetRsDCEstimationEnable(MCI_Handle_t* pHandle)
{
  uint32_t RetVal = MC_WRONG_STATE_ERROR;

  if (pHandle->pPolPulse->flagPolePulseActivation == false)
  {
...
MC_RetStatus_t MCI_EnablePulse(MCI_Handle_t* pHandle)
{
  uint32_t RetVal = MC_WRONG_STATE_ERROR;

  if (pHandle->pRsDCEst->flag_enableRSDCestimate == false)
  {
