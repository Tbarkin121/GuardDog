...
<#function GetSpeedSensor motor aux=false> 
      ...
      <#if motor == "M1"> 
        <#if !aux > <#assign entry = MC.M1_SPEED_SENSOR> <#else> <#assign entry = MC.M1_AUXILIARY_SPEED_SENSOR> </#if>
      <#else>
        <#if !aux > <#assign entry = MC.M2_SPEED_SENSOR> <#else> <#assign entry = MC.M2_AUXILIARY_SPEED_SENSOR> </#if>
      </#if>
...
<#if .vars["M" + MOTOR_NUM?c + "_IS_SENSORLESS"] == true>
...
<#if .vars.MC["M" + MOTOR_NUM?c + "_DRIVE_TYPE"] == "SIX_STEP">
        ...
        {
          uint8_t regdata8 = *data;
          PWMC_SetFastDemagState(&(&PWM_Handle_M${MOTOR_NUM})->_Super, regdata8);
          break;
        }
        ...
        {
          uint8_t regdata8 = *data;
          PWMC_SetQuasiSynchState(&(&PWM_Handle_M${MOTOR_NUM})->_Super, regdata8);
          break;
        }
...
<#if MEMORY_FOOTPRINT_REG>
  ...
  <#if .vars.MC["M"+ MOTOR_NUM?c + "_DBG_OPEN_LOOP_ENABLE"]>
...
<#if .vars["M" + MOTOR_NUM?c + "_IS_SENSORLESS"]>
            case MC_REG_RUC_STAGE_NBR:
            {
...
<#if .vars.MC["M" + MOTOR_NUM?c + "_DRIVE_TYPE"] == "SIX_STEP">
            case MC_REG_FAST_DEMAG:
            {
              *data = PWMC_GetFastDemagState(&(&PWM_Handle_M${MOTOR_NUM})->_Super);
              break;
            }

            case MC_REG_QUASI_SYNCH:  
            {
              *data = PWMC_GetQuasiSynchState(&(&PWM_Handle_M${MOTOR_NUM})->_Super);
              break;
            }
</#if><#-- DRIVE_TYPE == "SIX_STEP" -->
...
<#if MEMORY_FOOTPRINT_REG2>
  ...
  <#if MEMORY_FOOTPRINT_REG>
    ...
    <#if (.vars.MC["M" + MOTOR_NUM?c + "_SPEED_SENSOR"] == "SENSORLESS_ADC") || (.vars.MC["M" + MOTOR_NUM?c + "_SPEED_SENSOR"] == "SENSORLESS_COMP")>
            ...
            case MC_REG_BEMF_ZCR:
            {
              *regdataU16 = (uint16_t) BADC_GetBemfZcrFlag(&Bemf_ADC_M${MOTOR_NUM});
              break;
            }

            case MC_REG_BEMF_U:
            {
              *regdataU16 = BADC_GetLastBemfValue(&Bemf_ADC_M${MOTOR_NUM}, 0);
              break;
            }

            case MC_REG_BEMF_V:
            {
              *regdataU16 = BADC_GetLastBemfValue(&Bemf_ADC_M${MOTOR_NUM}, 1);
              break;
            }  

            case MC_REG_BEMF_W:           
            {
              *regdataU16 = BADC_GetLastBemfValue(&Bemf_ADC_M${MOTOR_NUM}, 2);
              break;
            }
...
<#if SIX_STEP>
  ...
  <#if (.vars.MC["M" + MOTOR_NUM?c + "_SPEED_SENSOR"] == "SENSORLESS_ADC") || (.vars.MC["M" + MOTOR_NUM?c + "_SPEED_SENSOR"] == "SENSORLESS_COMP")>      
          case MC_REG_BEMF_ZCR:
          {
            *dataPtr = &((&Bemf_ADC_M${MOTOR_NUM})->ZcDetected);
            break;
          }

          case MC_REG_BEMF_U:
          {
            *dataPtr = &((&Bemf_ADC_M${MOTOR_NUM})->BemfLastValues[0]);
            break;
          }

          case MC_REG_BEMF_V:
          {
            *dataPtr = &((&Bemf_ADC_M${MOTOR_NUM})->BemfLastValues[1]);
            break;
          }

          case MC_REG_BEMF_W:
          {
            *dataPtr = &((&Bemf_ADC_M${MOTOR_NUM})->BemfLastValues[2]);
            break;
          }
  </#if><#-- MC.SPEED_SENSOR == "SENSORLESS_ADC" -->
</#if><#-- SIX_STEP -->
