...
class DynasoarEnv:
    ...
    def step(self, actions):
        ...
        return  self.obs_buf, self.obs_buf_prev, self.rew_buf, self.reset_buf, self.kinematic_states, self.goal_states
    
    def init_data_buffers(self, cfg):
        self.num_envs     = cfg['env']['num_envs']
        self.num_acts     = cfg['env']['numActions']           # Roll(1), Pitch(1), MotorTorque(1), Prop Pitch(1)
        self.num_obs      = cfg['env']['numObservations']      # Height(1), Ground Speed(1), Air Speed(1), Ori(3), Battery(1), Vec2Target(2), Dist2Target(1), PropSpeed(1), MotorPower(1), Prop Pitch(1)
        ...
        self.obs_buf_prev = torch.zeros( (self.num_envs, self.num_obs), dtype=torch.float)
    ...
    def init_extras(self, cfg):
        ...
        self.max_episode_length = int(self.max_episode_length_s / self.dt + 0.5)*20
    ...
    def get_observations(self):
        ...
        self.obs_buf_prev[:] = self.obs_buf.clone()
...
def calc_rewards_jit(
    ...
    reward = torch.ones((num_envs), device='cuda') + bank_punishment
